...
public class TestTPM {
    ...
    private static final Logger log = LoggerFactory.getLogger(TestTPM.class);
    ...
    void testTpm() throws InterruptedException {
        ...
        while(h < 3) {
            ...
            while (i < epochs) {
                ...
                log.info("Эпоха: {}, out1: {}, out2: {}", i, out1, out2);
            ...
            log.info("Сгенерированный ключ 1: {}", t1.getSecretKey());
            log.info("Сгенерированный ключ 2: {}", t2.getSecretKey());
            log.info("Ключ после склеивания и приведения к байтам: {}", Encrypter.toBytes(t1.getSecretKey()));
